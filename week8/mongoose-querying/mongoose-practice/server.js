...
Book.find({pages: {$lt: 500}, pages: {$gt: 200}})
.then(books => {
  // console.log(books);
})
...
Book.find({genres: {$in: ["Fiction"]}}).skip(2).limit(3)
.then(books => {
  // console.log(books);
})
...
Person.find({$or: [{hair: "grey"},{weight: {$lt: 70}}]})
.then(people => {
  // console.log(people);
})
...
Person.find({weight: {$gt: 100}, kids: {$elemMatch: {weight: {$gt: 100}}}})
.then(people => {
  // console.log(people);
})